...
const isDutchFlagArray = (arr, flag) => {
  const lessEqualCounts = arr.reduce(
    (acc, curr) => {
      if (curr < flag) {
        acc[0] += 1;
      }
  ...
  const lessItemsRight = arr
    .slice(0, lessEqualCounts[0])
    .every((item) => item < flag);
  const equalRight = arr
    .slice(lessEqualCounts[0], lessEqualCounts[1])
    .every((item) => item === flag);
  const bigItemsRight = arr
    .slice(lessEqualCounts[0] + lessEqualCounts[1])
    .every((item) => item > flag);
...
test("", () => {
  ...
  for (let index = 0; index < testCount; index++) {
    const arr = Array.from(new Array(arrayLength), () =>
      parseInt(String(Math.random() * maxArrayValue))
    );
    const copyArr = [...arr];
